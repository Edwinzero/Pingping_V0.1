...
public:
	...
	int nodeID;				// node computer ID
	int deviceID;			// local device ID

	Sensor() :sensorID(-1), nodeID(-1), deviceID(-1){
...
public:
	...
	void send_config(int port){
		...
		Timer speed;
		char filepath[128];
		sprintf(filepath, "CapturedData/config_%d.txt", config::getNODEFROMPORT(port));
		speed.Start();
	...
	// Requset Parital_sensor info from each node computer and then merge them together from application layer
	void requestDevice(std::vector<Sensor> sensors, int port){
		boost::system::error_code error;
		message_.assign("rdv", 4);
		LOGOUT(message_);
		boost::asio::write(socket_, boost::asio::buffer(message_),
			boost::asio::transfer_all(), error);
		Timer speed;
		char filepath[128];
		sprintf(filepath, "CapturedData/master.config");
		speed.Start();
		//send_buffer(socket_, filepath);
		speed.Print("SENDING CONFIG FILE TIME IN (ms): ");
		// READ CONFIG CONTENT HERE...

		// recv device info (4 bytes local kid, 12 bytes k serial)
		boost::asio::streambuf deviceinfo_buf;
		{
			boost::asio::streambuf totalk_buf;
			int totalK = 0;
			size_t len = boost::asio::read(socket_, totalk_buf, boost::asio::transfer_exactly(4), error);
			std::istream totalk_istream(&totalk_buf);
			totalk_istream.read(reinterpret_cast<char*>(&totalK), 4);

			len = boost::asio::read(socket_, deviceinfo_buf, boost::asio::transfer_exactly(16*totalK), error);
			if (len == 0){
				return;
			}
			std::cout << "[RECV DEVICE INFO DATA] :: ->" << len << std::endl;
			std::cout << "[1]  DEVICE INFO stream buf size is " << deviceinfo_buf.size() << std::endl;
			std::istream request_stream(&deviceinfo_buf);
			std::vector<std::string> tmp_list; std::vector<int> tmp_id;
			tmp_list.resize(totalK);
			tmp_id.resize(totalK);
			for (int i = 0; i < deviceinfo_buf.size() / 16; +i){
				char tmp_serial[12];
				request_stream.read(reinterpret_cast<char*>(&tmp_id[i]), 4);
				request_stream.read(reinterpret_cast<char*>(&tmp_serial), 12);
				tmp_list[i] = tmp_serial;
				std::cout << "[KINECT ID] No." << i <<":: Local ID is:  " << tmp_id[i] << std::endl;
				std::cout << "[KINECT SERIAL] :: SERIAL NUMBER is: " << tmp_serial << std::endl;
				std::cout << "[KINECT SERIAL] :: SERIAL NUMBER is: " << tmp_list[i] << std::endl;
			}

			for (int i = 0; i < tmp_list.size(); ++i){
				for (int j = 0; j < sizeof(config::SERIAL_K); ++j){
					if (tmp_list[i] == config::SERIAL_K[j]){
						Sensor tmp_sensor;
						tmp_sensor.sensorID = j;
						tmp_sensor.deviceID = tmp_id[i];
						tmp_sensor.nodeID = config::getNODEFROMPORT(port);
						sensors.push_back(tmp_sensor);
						// sort?
						// if new serial found?
					}
				}
			...
			for (int i = 0; i < sensors.size(); ++i){
				std::cout << "[PARTIAL_SENOR INFO] :: -> global ID is:  <<< " << sensors[i].sensorID << "  >>>, deviceID(local) is " << sensors[i].deviceID
					<< " :: nodeID is " << sensors[i].nodeID << " !!!" << std::endl;
			}
		}
	}

	void request_frame(int id, std::vector<char> &buffer){
		boost::system::error_code error;
		//send rqs
		message_.assign("rqs", 4);
		boost::asio::write(socket_, boost::asio::buffer(message_), boost::asio::transfer_all(), error);

		//recv frame
		recv_buffer(socket_, id, buffer, 0);
	...
	template<class T>
	void request_frame(int id, std::vector<T> &buffer, size_t requestSize = 0){
		boost::system::error_code error;
		//send
		message_.assign("rqs", 4);
		boost::asio::write(socket_, boost::asio::buffer(message_), boost::asio::transfer_all(), error);
		...
		//recv frame
		recv_buffer(socket_, id, buffer, requestSize);
		std::cout << "[FRAME INFO] :: [BUFFER SIZE] => ---- <<<" << requestSize << " >>> ----" << std::endl;
...
public:
	...
	void send_buffer(boost::asio::ip::tcp::socket& socket, char filepath[]){
		...
		LOGOUT(filepath);
		LOGOUT(filesize);
		std::cout << "[1]  HEADER stream buf size is " << file_request.size() << std::endl;
	...
	void recv_buffer(boost::asio::ip::tcp::socket& socket){//, char filepath[]){
		...
		// Recv data
		boost::system::error_code error;
		size_t len = boost::asio::read(socket, boost::asio::buffer(buffer_.data(), buffer_.size()), error);
		if (len > 0) {
			// Store data
	...
	template< class T >
	bool recv_buffer(boost::asio::ip::tcp::socket& socket,int id, std::vector<T>& buffer, size_t requestSize){
		//
		boost::asio::streambuf header_buf;
		{
			size_t len = boost::asio::read(socket, header_buf, boost::asio::transfer_exactly(4));
			if (len != 4){
				return false;
			}
			std::cout << "[RECV IMG HEADER DATA] :: ->" << len << std::endl;
		}
		...
		std::istream request_stream(&header_buf);
		int file_size = 0; 
		request_stream.read(reinterpret_cast<char*>(&id), 4);
		request_stream.read(reinterpret_cast<char*>(&file_size), 4);
		std::cout << "[KINECT ID] :: is:  " << id << std::endl;
		std::cout << "[filesize] :: file size is: bytes " << file_size << std::endl;
		if (requestSize > 0 && requestSize != file_size){
			return false;
		}
		...
		{
			boost::system::error_code error;
			size_t len = boost::asio::read(socket, boost::asio::buffer(buffer.data(), (std::streamsize)(file_size)), boost::asio::transfer_all(), error);
			if (len != file_size){
				return false;
			}
			if (error){
			...
			std::cout << "[SYSTEM] :: [RCV IMG CONTENT] SUCCESS... " << error << std::endl;
		}
		return true;
	}
...
public:
	...
	void parsing_cmd(const std::string& cmd) {
		...
		if (cmd_ == "get imgs") {
			...
			// rqt
			std::vector<char> test_buf;
			size_t test_size = 0;
			image_session_->request_frame(id_, test_buf, test_size);
		}
		else if (cmd_ == "request device"){
			std::cout << "[SYSTEM] :: SENDING 'REQUEST DEVICE INFO FROM ALL NODES' COMMAND... " << std::endl;
			// rdv
			image_session_->requestDevice(sensors_, port_);
		}
...
int main()
{
	try
	{
		std::cout << sizeof(config::SERIAL_K) << std::endl;
...
#define TEST
#ifdef TEST
		tcp_server server0(io_service, config::PORT[0], config::NODE[0]);
#else
		tcp_server server0(io_service, config::PORT[0], config::NODE[0]);
		tcp_server server1(io_service, config::PORT[1], config::NODE[1]);
		//tcp_server server2(io_service, config::PORT[2], config::NODE[2]);
		//tcp_server server3(io_service, config::PORT[3], config::NODE[3]);
#endif
